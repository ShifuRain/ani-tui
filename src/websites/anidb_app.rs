...
use crate::websites::curl_client::{encode_query_param, CurlClient, QueryError};
...
/// binaries [`CurlClient`] looks for emulate.
const AGENT: &str =
...
impl AnimeRepository for AnidbApp {
    ...
    async fn detail(&self, raw_id: &str) -> anime_repo::Result<anime_repo::Detail> {
        ...
        let languages = match episodes.first() {
            Some(first) => {
                let url = format!("{BASE_URL}/api/frontend/episode/{}/languages", first.id.raw);
                self.client
                    .get(&url, &[])
                    .await
                    .ok()
                    .and_then(|json| parse_all_language_codes(&json).ok())
                    .unwrap_or_default()
            }
            None => Vec::new(),
        };
    ...
    async fn watch_link(&self, raw_id: &str) -> anime_repo::Result<WatchLink> {
        ...
        let embed_html = self.client.get(&embed_url, &[]).await?;

        let link = parse_embed_page(&embed_html).ok_or(ParsingError)?;
        Ok(WatchLink { url: link, headers: vec![] })
    }
}
...
/// Parses the languages JSON API response into every available language `code`. Split out from
/// [`AnidbApp::detail`] so it can be tested against a fixture file without a network
/// round-trip.
fn parse_all_language_codes(json: &str) -> Result<Vec<String>, QueryError> {
    let response: LanguagesResponse = serde_json::from_str(json).map_err(|_| ParsingError)?;
    Ok(response.languages.into_iter().map(|entry| entry.code).collect())
}
...
mod tests {
    ...
    #[test]
    fn parses_all_language_codes_fixture() {
        let json = include_str!("../../tests/fixtures/anidb-languages.json");

        let mut codes = parse_all_language_codes(json).expect("fixture should parse");
        codes.sort();
        assert_eq!(codes, vec!["eng".to_string(), "jpn".to_string()]);
    }
